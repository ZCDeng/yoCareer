...
yoCareer превращает любой AI-CLI в полноценный командный центр для поиска работы. Вместо ручного ведения таблиц с откликами вы получаете ИИ-пайплайн, который:
...
> **Важно: это НЕ инструмент для массовой рассылки.** yoCareer — это фильтр. Он помогает найти те немногие вакансии, которые стоят вашего времени, из сотен. Система настоятельно рекомендует не откликаться на вакансии с оценкой ниже 4.0/5. Ваше время ценно, как и время рекрутера. Всегда проверяйте перед отправкой.
...
# 6. Начинай
# Вставь URL вакансии или запусти /yoCareer
```

## Использование

yoCareer — одна slash-команда с множеством режимов:

```
/yoCareer                       → Показать все доступные команды
/yoCareer {вставь описание JD}  → Полный авто-пайплайн (оценка + PDF + трекер)
/yoCareer scan                  → Сканирование порталов
/yoCareer pdf                   → Генерация ATS-резюме
/yoCareer batch                 → Пакетная оценка вакансий
/yoCareer tracker               → Статус откликов
/yoCareer apply                 → Заполнение форм отклика с ИИ
/yoCareer pipeline              → Обработка очереди URL
/yoCareer contacto              → Сообщение в LinkedIn
/yoCareer deep                  → Глубокий анализ компании
/yoCareer training              → Оценка курса/сертификации
/yoCareer project               → Оценка портфолио-проекта
```

Или просто вставь URL вакансии — yoCareer автоматически определит его и запустит полный пайплайн.
...
yoCareer поддерживает ключевые российские джоб-борды:
...
**yoCareer — локальный open-source инструмент, НЕ хостинг-сервис.** Используя это ПО, вы подтверждаете:
