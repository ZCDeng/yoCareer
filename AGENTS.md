...
Canonical agent-agnostic instructions. All compatible AI coding CLIs (Claude Code, Codex, Gemini CLI, OpenCode, Qwen Code, Copilot CLI, etc.) read this file. Tool-specific shims (`CLAUDE.md`, `GEMINI.md`) import this file via `@AGENTS.md` and only add platform-specific notes.

## Origin

This system was built and used by [ZCDeng](https://github.com/ZCDeng) to evaluate 740+ job offers, generate 100+ tailored CVs, and land an Applied AI lead role. The archetypes, scoring logic, negotiation scripts, and proof point structure all reflect a real career search in AI/automation roles.

The portfolio that goes with this system is also open source: [cv-santiago examples](https://github.com/ZCDeng/yoCareer/tree/main/examples).

**It will work out of the box, but it's designed to be made yours.** If the archetypes don't match your career, the modes are in the wrong language, or the scoring doesn't fit your priorities -- just ask. The agent can edit the user's files. The user says "change the archetypes to data engineering roles" and the agent does it. That's the whole point.

## Multi-CLI Support

yoCareer follows the [agentskills.io](https://agentskills.io) open standard. The single source of truth for the `/yoCareer` skill lives at `.agents/skills/yoCareer/SKILL.md`. Compatible CLIs:

| CLI | Skill location | Context file |
|-----|----------------|--------------|
| Claude Code | `.claude/skills/yoCareer/SKILL.md` (mirror) | `CLAUDE.md` → `@AGENTS.md` |
| Codex | `.agents/skills/yoCareer/SKILL.md` | `AGENTS.md` (this file) |
| Gemini CLI | `.agents/skills/yoCareer/SKILL.md` | `GEMINI.md` → `@AGENTS.md` |
| OpenCode | `.agents/skills/yoCareer/SKILL.md` | `AGENTS.md` (this file) |
| Qwen Code | `.agents/skills/yoCareer/SKILL.md` | `AGENTS.md` (this file) |
| Copilot CLI | `.agents/skills/yoCareer/SKILL.md` | `AGENTS.md` (this file) |

The skill exposes a single command (`/yoCareer`) that routes to mode handlers in `modes/`. Per-platform legacy slash command files (`.gemini/commands/*.toml`, `.opencode/commands/*.md`) have been removed in favor of this unified skill router.
...
- `AGENTS.md` (this file), `CLAUDE.md`, `GEMINI.md`, `*.mjs` scripts, `dashboard/*`, `templates/*`, `batch/*`
...
> "I'll set up the job scanner with the China-first portal templates (国内 AI/科技公司 + 微博 / 小红书 / V2EX / GitHub 信号入口). Want me to customize the search keywords for your target roles?"

Copy `templates/portals.cn.example.yml` (preferred for China users) or `templates/portals.example.yml` → `portals.yml`. If they gave target roles in Step 2, update `title_filter.positive` to match.
...
> - Run `/yoCareer scan` to search portals
...
If the user accepts and the host CLI provides a scheduling primitive (`/loop` / `/schedule` / cron), use it to run `/yoCareer scan` periodically. Otherwise suggest a system cron job or remind them to run `/yoCareer scan` manually.

### Personalization

This system is designed to be customized by the AI agent. When the user asks to change archetypes, translate modes, adjust scoring, add companies, or modify negotiation scripts -- do it directly. The agent reads the same files it uses, so it knows exactly what to edit.
...
**NEVER trust generic web search/fetch to verify if an offer is still active.** ALWAYS use a real browser (Playwright MCP or equivalent):
1. `browser_navigate` to the URL
2. `browser_snapshot` to read content
3. Only footer/navbar without JD = closed. Title + description + Apply = active.

**Exception for batch workers (`claude -p` and equivalents):** Playwright is not available in headless pipe mode. Use WebFetch as fallback and mark the report header with `**Verification:** unconfirmed (batch mode)`. The user can verify manually later.
...
- Node.js (mjs modules), Playwright (PDF + scraping), YAML (config), HTML/CSS (template), Markdown (data), optional MCP integrations (Aditly bridge, draw.io, Canva)
...
## China-First Provider Architecture (yoCareer fork)

The scanner pipeline is structured around China job market realities. Provider order in `portals.yml`:

1. **`company_page`** — Direct scrape of company career pages via Playwright. Highest signal-to-noise.
2. **`manual_signal_import`** — Hand-curated signals from `data/signals.ndjson` (微信公众号, 招聘群, 私域消息).
3. **`reach_signal_search`** — Public signal search bridge (微博 / 小红书 / V2EX / GitHub) with optional Aditly upgrade.
4. **`manual_only`** — Login-walled / risk-controlled platforms (BOSS直聘 / 智联 / 前程无忧 / 猎聘 / 拉勾) default to human-in-the-loop import + review.

### Optional: Aditly Bridge

Public-signal scraping quality (微博 / 小红书 / V2EX / GitHub) can be upgraded by attaching [Aditly](https://github.com/ZCDeng/Aditly) as an external capability. The integration is "external-first + local fallback" — when Aditly is unavailable, the scanner automatically degrades to local bridge logic. Configure via `.env`:
...
Verify with `npm run providers` and `npm run bridge:smoke`. See `docs/ADITLY_INTEGRATION.md` for the full setup guide.
