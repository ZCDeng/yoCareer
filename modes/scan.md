...
> **Nota yoCareer:** El escáner por defecto (`scan.mjs` / `npm run scan`) ya es una tubería de providers: conserva las APIs públicas de Greenhouse, Ashby y Lever, añade `company_page` con Playwright para páginas públicas de empresas, y acepta `manual_signal_import` desde `data/signals.ndjson` para señales fragmentadas provistas por el usuario. Los flujos de WebSearch/Reach/social descritos abajo siguen siendo extensiones de agente o providers futuros.
...
→ Ejecuta /yoCareer pipeline para evaluar las nuevas ofertas.
```
