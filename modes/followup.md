...
> "No active applications to follow up on. Apply to some roles first with `/yoCareer` and come back when they're aging."
...
- Suggest the user run `/yoCareer contacto {company}` to find the right person first
...
> - Trying a different contact via `/yoCareer contacto`
...
**To:** {email or "No contact found — run `/yoCareer contacto` first"}
