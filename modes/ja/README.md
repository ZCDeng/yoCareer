# yoCareer -- 日本語モード (`modes/ja/`)

このフォルダには、日本市場または日本語で運営される企業での求職活動を行う候補者向けに、yoCareer の主要モードを日本語に翻訳したファイルが含まれています。
...
yoCareer にはコード上の「言語スイッチ」フラグはありません。代わりに 2 つの方法があります：
