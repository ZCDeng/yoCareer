...
`data/pipeline.md` に蓄積された求人 URL を処理する。候補者がいつでも URL を追加し、後から `/yoCareer pipeline` を実行してまとめて処理する。
...
3. **3 つ以上の URL がある場合**、ホスト CLI のサブエージェント機能（例: Claude Code の `Agent`/`Task`、Codex の `spawn_agent`、バックグラウンドタスク）でエージェントを並列起動して速度を最大化。
4. **完了後**、サマリーテーブルを表示：
