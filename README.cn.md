...
| **Dashboard TUI** | 在终端 UI 中浏览、筛选和排序你的求职管道（遗留，不再主动维护）|
...
# 6. 启动 daemon 和 Web UI（推荐）
npm run daemon   # 启动 HTTP 服务 localhost:8650
npm run ui       # 打开浏览器仪表盘
...
├── daemon/                      # v2 HTTP 服务（REST API + SSE）
...
- **Dashboard（遗留）**：Go + Bubble Tea + Lipgloss（Catppuccin Mocha 主题）
