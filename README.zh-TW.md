...
[English](README.md) | [简体中文](README.cn.md) | [繁體中文](README.zh-TW.md) | [項目主頁](README.yocareer.md)

> **v2.0.0 更新**：全新 daemon + SQLite 架構、瀏覽器 SPA 儀表板、瀏覽器擴充套件、Mirofish 設計系統。
...
| **瀏覽器 SPA 儀表板** | Dark-first 網頁儀表板，Cmd+K 命令面板，SSE 即時更新 |
| **瀏覽器擴充套件** | Manifest V3，一鍵提取 BOSS直聘 / 拉勾 / 智聯職位資訊 |
| **儀表板 TUI** | 在終端機 UI 中瀏覽、篩選及排序你的求職管道（遺留，不再主動維護）|
...
# 6. 啟動 daemon 和 Web UI（推薦）
npm run daemon   # 啟動 HTTP 服務 localhost:8650
npm run ui       # 開啟瀏覽器儀表板
...
Dark-first 網頁儀表板，功能包括：
- **模組卡片**：Profile / Portals / Signals / Applications / Evaluations
- **Cmd+K 命令面板**：快速導航、觸發掃描、切換模組
- **SSE 即時更新**：任務進度、信號變化自動重新整理
- **Mirofish 設計系統**：一致的顏色、間距、動畫令牌

### 瀏覽器擴充套件（可選）

一鍵提取 BOSS直聘 / 拉勾 / 智聯職位資訊到 daemon：

1. 確保 daemon 正在執行（`npm run daemon`）
2. Chrome → `chrome://extensions` → 開啟開發者模式
3. 載入未封裝項目 → 選擇 `extension/` 目錄
4. 在支援的頁面點擊擴充套件圖示提取職位

首次使用需配對：擴充套件彈出面板輸入 6 位配對碼。

### 遺留儀表板 TUI

Go TUI 儀表板仍保留在 `dashboard/` 中，但不再主動維護：
...
├── daemon/                      # v2 HTTP 服務（REST API + SSE）
│   ├── lib/                     # SQLite schema + helpers
│   └── routes/                  # API 路由
├── modes/                       # 技能模式（EN + zh-CN）
│   ├── _shared.md               # 共用情境
│   ├── oferta.md                # 單一職缺評估
...
│   ├── batch.md                 # 批次處理
│   └── zh-cn/                   # 中文模式
├── web-ui/                      # v2 瀏覽器 SPA 儀表板
│   ├── tokens.css               # Mirofish 設計令牌
│   ├── cmdk.js                  # Cmd+K 命令面板
│   └── main.js                  # 模組路由
├── extension/                   # v2 瀏覽器擴充套件（MV3）
│   ├── manifest.json
│   ├── popup/
│   └── content/
├── lib/                         # v2 共享函式庫
│   ├── daemon-client.mjs        # HTTP 客戶端
│   ├── ensure-daemon.mjs        # 同步啟動輔助
│   └── db-helpers.mjs           # SQLite 輔助
├── templates/
...
- **代理**：Claude Code / Gemini CLI / Codex / OpenCode / Qwen Code / Copilot CLI（agentskills.io 開放標準）
- **後端**：Node.js HTTP daemon（localhost:8650）、SQLite + WAL、better-sqlite3、REST API + SSE
- **前端**：Vanilla JS SPA、Mirofish 設計系統、Fuse.js Cmd+K、原生 EventSource
- **擴充套件**：Chrome Extension Manifest V3、content script 提取器
...
- **資料**：SQLite（主儲存）+ Markdown/YAML（使用者層）+ TSV 批次檔案
- **儀表板（遺留）**：Go + Bubble Tea + Lipgloss（Catppuccin Mocha 主題）
