...
## Execution Status (Shipped 2026-05-10)

All 9 implementation units completed and merged to `main`. VERSION bumped to `2.0.0`.

| Unit | PR | Status | Actual vs Plan Delta |
|------|-----|--------|----------------------|
| U1 SQLite foundation + state machines + stale propagation | #33 | Merged | Schema simplified: 6 entities (profiles, portals, signals, applications, evaluations, tasks) instead of 8; `event_log` as JSON column on each table; `task_runs` table covers G6 cooperative checkpoint |
| U2 Daemon HTTP server + token auth + CORS + SSE | #34 | Merged | Port 8650 default (not 8643); pairing code 6-digit flow confirmed; SSE ticket-based auth confirmed working; `lib/ensure-daemon.mjs` added for synchronous auto-start |
| U3 Domain layer + 6 entity REST + task_runner | #35 | Merged | `daemon/lib/` holds schema + helpers (not `daemon/models/`); `pdf-extract.mjs` migrated to daemon HTTP upsert; 6 entity routes + tasks route implemented |
| U4 CLI thin-wrapper migration | #36 | Merged | 11 entry-point scripts wired to `lib/daemon-client.mjs` + `lib/ensure-daemon.mjs`; `daemon-cli.mjs` provides `start/stop/status`; all scripts auto-start daemon if not running |
| U5 Web SPA shell + Cmd+K + SSE client | #37 | Merged | Module cards (Profile/Portals/Signals/Applications/Evaluations) confirmed; Cmd+K via Fuse.js; SSE real-time updates confirmed; served by daemon static file handler |
| U6 Mirofish design tokens + dark-first visual | #38 | Merged | 3-tier CSS token system (`--mf-*`) in `web-ui/tokens.css`; 4-file design matrix in `templates/mirofish-system/`; dark-first default with light override via `prefers-color-scheme` |
| U7 Browser Extension (MV3) | #37 | Merged | Manifest V3 confirmed; content script extraction for BOSS/拉勾/智联; popup + 6-digit pairing flow; service worker HTTP communication with daemon |
| U8 CN boundary hardening + capabilities + v1 detect | #38 | Merged | `modes/{de,fr,ja,pt,ru}/` + 5 non-CN READMEs deleted; `templates/capabilities.yml` created; `lib/v1-detect.mjs` added; v1-to-v2 upgrade detection confirmed |
| U9 Test infra + docs | #38, #39, #40 | Merged | test-all.mjs Section 16 added (v2 architecture checks: daemon paths, extension manifest V3, token definitions, capabilities version, v1-detect exports); docs/ARCHITECTURE.md, SETUP.md, SCRIPTS.md updated in #39; README.cn.md, README.zh-TW.md synced in #40 |

### Post-Ship Items

- **Dashboard Go TUI:** Retained in `dashboard/` (not deprecated); no longer actively maintained but still buildable
- **docs/solutions/ learnings:** Deferred per original plan — to be populated after 30-day observation period
- **Extension distribution:** Edge Add-ons / 极简插件商店 / Crx搜搜 listing pending (not blocking v2.0.0)
- **Mirofish hero placeholder SVG:** Still placeholder; final designer assets to be delivered in follow-up PR
